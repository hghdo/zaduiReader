...
class RSSHandler extends org.xml.sax.helpers.DefaultHandler {
	...
	private final Setter ADD_MEDIA_THUMBNAIL = new AttributeSetter() {
		...
		public void set(org.xml.sax.Attributes attributes) {
			...
			item.addThumbnail(new MediaThumbnail(android.net.Uri.parse(url),
					height, width));
		}

	};
	
	private final Setter SET_THUMB_URL = new ContentSetter() {

		@Override
		public void set(String thumbUrl) {
			if (item != null) item.setThumbUrl(thumbUrl);
		}
	};
	
	private final Setter SET_ZIP_PKG_URL=new ContentSetter(){
		@Override
		public void set(String zipPkgUrl){
			if(item!=null) item.setZipPkgUrl(zipPkgUrl);
		}
	};
	...
	private final Setter SET_GUID=new ContentSetter(){
		...
		public void set(String guid){
			if(item!=null) item.setGuid(guid);
		}		
	};
	 ...
	 * 
	 * @param categoryAvg
	 *            average number of RSS item &lt;category&gt; elements
	 * @param thumbnailAvg
	...
	RSSHandler(RSSConfig config) {
		...
		setters.put("zipPkgUrl", SET_ZIP_PKG_URL);
		setters.put("thumbUrl", SET_THUMB_URL);
	}
